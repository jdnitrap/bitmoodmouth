...
Context-mixing bit predictor + ImHex-style grid specialist (E).
"""
...
class Model:
    ...
    def __init__(self) -> None:
        ...
        self.grid_table: Dict[Tuple, Adaptive] = defaultdict(Adaptive)
        self.mixer = Mixer(len(self.ORDERS) + 1 + N_GRID)
    ...
    def _ensure_grid(self) -> None:
        ...
        if not hasattr(self, "use_grid"):
            self.use_grid = True
        self.mixer.ensure(len(self.ORDERS) + 1 + N_GRID)
    ...
    def predict_bit(self, bit_index: int, partial: int):
        self._ensure_grid()
        cells: List[Adaptive] = []
        ...
        return self.mixer.mix(stretched), stretched, cells
...
def sample_byte(mass, rng, temp, top_k, charset, seen) -> int:
    weights = []
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    print("specialists    O0 O1 O2 O3 O4 C E(line,auto0,auto1 x 2 votes)")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    return {"train": cmd_train, "generate": cmd_generate, "info": cmd_info, "write": cmd_write}[args.cmd](args)
